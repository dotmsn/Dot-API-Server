...
export class FriendsService {
  ...
  public async getSpecificRequest(
    from: string,
    to: string,
  ...
  public async acceptFriendRequest(
    ...
    if (request == null) {
      throw new BadRequestException(
        ...
        'This friend request is invalid or has expired.',
      );
    } else if (request.to != userID) {
      throw new UnauthorizedException(
        'FRIEND_REQUEST_NOT_FOR_YOU',
        "This friend isn't to you.",
      );
    } else {
  ...
  public async create(from: string, to: string): Promise<FriendRequest> {
    if (from == null || to == null) {
      ...
      throw new BadRequestException(
        ...
        'You cannot sent a friend request yourself.',
      );
    } else if (await this.getSpecificRequest(from, to)) {
      throw new NotAcceptableException(
        'ALREADY_SENT_REQUEST',
        'You has already sent a friend request to this user.',
      );
    } else if (await this.getSpecificRequest(to, from)) {
      throw new NotAcceptableException(
        'ALREADY_INCOMING_REQUEST',
        'You has already an incoming friend request from this user.',
      );
    } else {
